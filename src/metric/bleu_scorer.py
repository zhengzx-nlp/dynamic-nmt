...
REPLACE_UNICODE_PL = os.path.join(os.path.dirname(__file__),
                                  "scripts/tokenizer/replace-unicode-punctuation.perl")
TOKENIZER_PL = os.path.join(os.path.dirname(__file__), "scripts/tokenizer/tokenizer.perl")
...
class SacreBLEUScorer(object):
    ...
    def __init__(self, reference_path, lang_pair, sacrebleu_args=None, t2t_bleu=False, postprocess=False, num_refs=1, test_set=None):
        """Initialize Scorer
    ...
    def _postprocess_t2t_bleu(self, stdin):
        cmd_replace_unicode = subprocess.Popen(
            ["perl", REPLACE_UNICODE_PL, "-l", self.tgt_lang],
            stdin=stdin, stdout=subprocess.PIPE, stderr=DEVNULL
        )
        cmd_tokenize = subprocess.Popen(
            ["perl", TOKENIZER_PL, "-l", self.tgt_lang],
            stdin=cmd_replace_unicode.stdout,
            stdout=subprocess.PIPE, stderr=DEVNULL
        )
        cmd_convert_to_atat = subprocess.Popen(
            ["perl", "-ple", "s{(\S)-(\S)}{$1 ##AT##-##AT## $2}g"],
            stdin=cmd_tokenize.stdout,
            stdout=subprocess.PIPE, stderr=DEVNULL
        )
        return cmd_convert_to_atat

    def _compute_bleu(self, stdin):
        cmd_bleu = subprocess.Popen(["sacrebleu"] + self.sacrebleu_args + ["--score-only",] + self.references,
                                    stdin=stdin,
    ...
    def corpus_bleu(self, hyp_in):
        ...
        if self.t2t_bleu:
            print("-"*100)
            inp = self._postprocess_t2t_bleu(inp).stdout
...
if __name__ == "__main__":
    bs = SacreBLEUScorer(
          "/home/public_data/nmtdata/wmt14_en-de_clean/newstest2014.de.tok.atat",
          "en-de",
          "--tokenize none",
          True,
          True,
